"""
Prescription extraction test — Bedrock Converse API.

Runs a prescription image or PDF through a Bedrock model and prints the
structured JSON it extracts. Use it to compare models before wiring one
into the agent.

    pip install boto3
    python test_extraction.py demo-data/rx1.jpg
    python test_extraction.py demo-data/rx1.jpg --model smart
    python test_extraction.py demo-data/rx1.jpg --model us.amazon.nova-pro-v1:0

No AWS config beyond `aws configure` — it uses your default credentials.
"""
...
def build_content_block(path: str) -> dict:
    ...
    if ext == "pdf":
        ...
        safe_name = re.sub(r"[^A-Za-z0-9 \-()\[\]]", "", os.path.basename(path)) or "prescription"
    ...
    guessed = mimetypes.guess_type(path)[0] or "unknown"
    sys.exit(f"Unsupported file type '{ext}' ({guessed}). Use jpg, png, gif, webp or pdf.")


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("file", help="Path to the prescription image or PDF")
    ap.add_argument("--model", default="fast",
                    help="fast | smart | nova, or a full Bedrock model/profile ID")
    ...
    try:
        response = client.converse(
            ...
            messages=[{"role": "user",
                       "content": [build_content_block(args.file), {"text": USER_PROMPT}]}],
            inferenceConfig={"maxTokens": 2000, "temperature": 0},
        )
    except ClientError as e:
        code = e.response["Error"]["Code"]
        sys.exit(f"\n{code}: {e.response['Error']['Message']}\n"
                 f"If AccessDenied, this model isn't invocable on your account — try another.")
    ...
    try:
        ...
        print(f"medicines     : {len(parsed.get('medicines', []))}")
        print(f"exam findings : {len(parsed.get('exam_findings', []))}")
        print(f"confidence    : {confidence}")
        if parsed.get("unreadable_fields"):
            print(f"unreadable    : {', '.join(parsed['unreadable_fields'])}")

        # This is the gate the agent must enforce: never proceed on a weak extraction.
        if confidence == "low" or not tests:
            print("\n>>> GATE: would route to NEEDS_HUMAN "
                  "(low confidence or no tests found)")
        else:
            print("\n>>> GATE: would proceed to LABS_SHORTLISTED")
    except json.JSONDecodeError:
        print("!! Model did not return valid JSON. Raw output:\n")
        print(text)
        print("\n>>> GATE: would route to NEEDS_HUMAN (unparseable output)")
